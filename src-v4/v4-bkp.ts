...
export type IsTupleSchema<T> = T extends readonly any[]
...
type CallArgsFromSchema<TSchema extends AnySchema | undefined> =
  TSchema extends StandardSchemaV1<infer UInput, any>
    ? UInput extends readonly any[]
      ? IsTupleSchema<UInput> extends true
        ? MakeUndefinedOptional<UInput> // tuple -> spread + optional defaults
...
//   const isStandardSchema = Boolean(schema?.["~standard"]);
//   let schemaToUse = schema as StandardSchemaV1;

//   if (
//     schema &&
//     isTypedError(inputToValidate) &&
//     !isStandardSchema && // then it's an errorsMap
//     inputToValidate?.type in schema
//   ) {
//     schemaToUse = (schema as any)[inputToValidate?.type] as StandardSchemaV1;
//   }

//   if (!schemaToUse) {
//     return createResult(null, new ZagoraError(`Invalid schema`), false);
//   }

//   const result = schemaToUse["~standard"].validate(inputToValidate);

//   if (result instanceof Promise) {
//     return result.then((res) => ({
//       data: res.issues ? null : res.value,
//       error: res.issues ? ZagoraError.fromIssues(res.issues, msg) : null,
//     }));
//   }
...
export class Builder<
  ...
  TErrors extends Record<string, StandardSchemaV1> | undefined = undefined,
  TCallArgs extends
    CallArgsFromSchema<TInputSchema> = CallArgsFromSchema<TInputSchema>,
