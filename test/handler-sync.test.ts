...
test("should handle handler throwing exceptions", () => {
  ...
  expect((result.error?.cause as Error).message).toContain("Some basic error");
});
...
test("should return error when async function is passed to .handler", async () => {
  const handler = zagora()
    .input(zodSchemas.string)
    .output(zodSchemas.string)
    .handler((input) => {
  ...
  expect(result.error).toBeInstanceOf(ZagoraError);
  expect(result.isDefined).toBe(false);
  expect(result.error?.message).toContain("only accepts synchronous functions");
});
...
test("should work with multiple error schemas", () => {
  ...
  const networkResult = procedure("network");
  if (!networkResult.error) {
    throw new Error("Expected error");
  }

  console.log(">>>>", networkResult.error, "<<<<");
  expect(networkResult.error.data).toBe("NETWORK_ERROR");
  // expect(networkResult.error.reason.toLowerCase()).toContain("network");
  // expect(networkResult.isDefined).toBe(true);

  // const validationResult = procedure("validation");
...
test("should reject async functions in sync handler", () => {
  ...
  expect(result.error?.message).toContain(
    "Using `.handler` only accepts synchronous functions",
  );
});
